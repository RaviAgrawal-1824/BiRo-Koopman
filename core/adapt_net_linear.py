...
class AdaptNet_linear(nn.Module):
    def __init__(self, adapt_net_params):
        super(AdaptNet_linear, self).__init__()
    ...
    def forward(self, data):
        ...
        del_z_pred = torch.matmul(Z, torch.transpose(del_A_matrix, 0, 1)) + torch.matmul(U, torch.transpose(del_B_matrix, 0, 1))
        return del_z_pred
    ...
    def loss(self, outputs, labels):
        ...
        loss = criterion(outputs, labels) + alpha * torch.norm(self.del_A_matrix, p=1) + alpha * torch.norm(self.del_B_matrix, p=1) + beta * torch.norm(self.del_A_matrix, p=2) + beta * torch.norm(self.del_B_matrix, p=2)
        return loss


    def model_pipeline(self, Z, U, del_Z, print_epoch = True):
        self.construct_adapt_net()
        self.set_optimizer_()
        X_train = np.concatenate((Z.T, U.T), axis=1)

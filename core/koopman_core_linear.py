...
class KoopmanNet_linear(nn.Module):
  ...
  def loss(self, outputs, labels):
    ...
    # print(n,n_z,outputs.shape,labels.shape)
    # print(sum(sum(np.isnan(outputs.detach().numpy()))))
    # print(sum(sum(np.isnan(labels.detach().numpy()))))
    # exit()
    # l1 regularization
    ...
    if override_C:
        pred_loss = criterion(x_prime_diff_pred,x_prime_diff)
    ...
    lifted_loss = criterion(z_prime_diff_pred, z_prime_diff)/(n_z/n)
...
class KoopmanNetCtrl_linear(KoopmanNet_linear):
    ...
    def forward(self, data):
        ...
        z_prime_diff_pred = torch.matmul(z, torch.transpose(drift_matrix, 0, 1)) + torch.matmul(u, torch.transpose(act_matrix, 0, 1))
    ...
    def construct_drift_act_matrix_(self): 
        ...
        const_obs_dyn_act = torch.zeros((first_obs_const, m)) # actuation for the constant term
    ...
    def process(self, data_x, data_u = None, downsample_rate = 1, train_mode =True):
        ...
        if train_mode:
            ...
            if n>9 and self.loss_scaler_x[10] < 0.001:
              self.loss_scaler_x[10] = 0.0011
            self.loss_scaler_z = np.std(x_prime_flat.T - x_flat.T)
            # print('loss_scaler_x:',self.loss_scaler_x,self.loss_scaler_x.shape)
            # print('loss_scaler_z:',self.loss_scaler_z,self.loss_scaler_z.shape)
...
class KoopDNN_linear():
  ...
  def model_pipeline(self, net_params, print_epoch = True, early_stop=False, plot_data = False):
    ...
    self.train_model(dataset_train, dataset_val, print_epoch=print_epoch, early_stop=early_stop)

  def train_model(self, dataset_train, dataset_val, print_epoch = True, tune_run = False, early_stop = False, early_stop_crit=1e-3, early_stop_max_count=8,):
    ...
    for epoch in range(self.net.net_params['epochs']):
      ...
      if epoch%10 == 0:
        lr=lr*0.8
        self.optimizer = optim.Adam(self.net.parameters(), lr=lr, weight_decay=self.net.net_params['l2_reg'])
        # self.optimizer = optim.SGD(self.net.parameters(), lr=lr, momentum=self.net.net_params['momentum'])
  ...
  # Print epoch loss
      ...
      if print_epoch:
          print('Epoch %3d: train loss: (%.10f,%.10f,%.10f), validation loss: (%.10f,%.10f,%.10f)' %(epoch + 1,running_loss,running_pred_loss,running_lifted_loss,val_loss,val_pred_loss,val_lifted_loss))
  ...
  def set_optimizer_(self):
    if self.net.net_params['optimizer'] == 'sgd':
        ...
        # self.optimizer_encoder = optim.SGD(self.net.opt_parameter_encoder, lr=lr, momentum=momentum)
